  ...
  val KUBERNETES_DRIVER_UI_SERVICE_ENABLED =
    ConfigBuilder("spark.kubernetes.driver.ui.service.enabled")
      .doc("If true, Spark will create a dedicated Kubernetes Service for the Spark driver " +
        "Web UI (separate from the headless driver service). This is useful when accessing the " +
        "Spark UI via an Ingress or LoadBalancer, since the headless driver service is not " +
        "suitable as an Ingress backend. When enabled, after the driver Web UI starts, Spark " +
        "will patch the Service's targetPort to match the actual bound UI port, which allows " +
        "using `spark.ui.port=0` (random port) or `spark.port.maxRetries` in hostNetwork mode " +
        "without breaking external UI access. Requires the driver's ServiceAccount to have " +
        "`patch services` permission.")
      .version("4.1.0")
      .booleanConf
      .createWithDefault(false)
  ...
  val KUBERNETES_DRIVER_UI_SERVICE_NAME =
    ConfigBuilder("spark.kubernetes.driver.ui.service.name")
      .doc("Optional override for the dedicated Spark driver Web UI Service name " +
        s"(only applies when ${KUBERNETES_DRIVER_UI_SERVICE_ENABLED.key}=true). " +
        "If unset, Spark derives the name as `<resourceNamePrefix>-ui-svc`. Useful when " +
        "an external controller (e.g. Spark Operator) expects a specific naming scheme " +
        "for its Ingress backend.")
      .version("4.1.0")
      .stringConf
      .createOptional
