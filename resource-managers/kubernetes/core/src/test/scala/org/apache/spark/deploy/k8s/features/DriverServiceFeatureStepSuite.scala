...
class DriverServiceFeatureStepSuite extends SparkFunSuite {
  ...
  test("SPARK-49407: dedicated UI service is not created when disabled by default.") {
    val sparkConf = new SparkConf(false).set(UI_PORT, 4080)
    val kconf = KubernetesTestConf.createDriverConf(sparkConf = sparkConf)
    val step = new DriverServiceFeatureStep(kconf)
    // Only the headless driver service is returned; UI service disabled by default.
    assert(step.getAdditionalKubernetesResources().size === 1)
    // Internal service-name property is not exposed when the feature is off.
    assert(!step.getAdditionalPodSystemProperties()
      .contains(DriverServiceFeatureStep.KUBERNETES_DRIVER_UI_SERVICE_NAME_INTERNAL))
  }

  test("SPARK-49407: dedicated UI service is created when enabled.") {
    val sparkConf = new SparkConf(false)
      .set(UI_PORT, 4080)
    ...
    // Headless driver-svc + dedicated ui-svc.
    assert(resources.size === 2)

    val services = resources.map(_.asInstanceOf[Service])
    val driverSvc = services.head
    val uiSvc = services(1)

    // headless driver-svc unchanged
    assert(driverSvc.getSpec.getClusterIP === "None")

    // Dedicated UI service is ClusterIP (default), exposes only the UI port, uses the same
    // selector as the driver pod, and starts with a placeholder targetPort (patched at runtime).
    ...
    // Internal service-name property is exposed for the driver-side patcher.
    assert(step.getAdditionalPodSystemProperties()
      .get(DriverServiceFeatureStep.KUBERNETES_DRIVER_UI_SERVICE_NAME_INTERNAL)
      === Some(s"${kconf.resourceNamePrefix}${DRIVER_UI_SVC_POSTFIX}"))
  }

  test("SPARK-49407: UI service honors explicit name and type overrides.") {
    val sparkConf = new SparkConf(false)
      .set(UI_PORT, 4080)
      .set(KUBERNETES_DRIVER_UI_SERVICE_ENABLED, true)
    ...
    val uiSvc = step.getAdditionalKubernetesResources()(1).asInstanceOf[Service]
