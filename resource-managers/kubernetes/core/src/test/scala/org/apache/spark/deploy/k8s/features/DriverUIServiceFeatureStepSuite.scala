...
class DriverUIServiceFeatureStepSuite extends SparkFunSuite {
  ...
  test("SPARK-49407: no additional resource when the feature is disabled") {
  ...
  test("SPARK-49407: independent of DriverServiceFeatureStep " +
    "(can be enabled even when DriverServiceFeatureStep is excluded)") {
    // Simulate hostNetwork scenario: user excludes DriverServiceFeatureStep and only wants
    // the dedicated UI service so an Ingress can front the driver's UI.
    val sparkConf = new SparkConf(false)
      .set(UI_PORT, 4080)
      .set(KUBERNETES_DRIVER_UI_SERVICE_ENABLED, true)
    val kconf = KubernetesTestConf.createDriverConf(sparkConf = sparkConf)
    ...
    // Regardless of whether DriverServiceFeatureStep runs, DriverUIServiceFeatureStep
    // produces exactly one ui-svc entry.
    val resources = step.getAdditionalKubernetesResources()
    assert(resources.size === 1)
    val uiSvc = resources.head.asInstanceOf[Service]
    assert(uiSvc.getSpec.getPorts.size === 1)
